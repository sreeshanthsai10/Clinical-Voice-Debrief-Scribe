...
class Medication(BaseModel):
    name: str = Field(description="Generic or brand name of medication")
    dosage: Optional[str] = Field(default=None, description="e.g., 500mg, 2 puffs, 10ml")
    frequency: Optional[str] = Field(default=None, description="e.g., Twice daily, PRN, every 6 hours")
    duration: Optional[str] = Field(default=None, description="e.g., 5 days, 10 days, 2 weeks")

class SafetyAlert(BaseModel):
    severity: str = Field(description="'HIGH' for absolute contraindications/allergies, 'MODERATE' for precautions")
    medication: str = Field(description="The medication triggering the alert")
    warning: str = Field(description="Clinical explanation of interaction, allergy, or dosing risk")

class SOAPNote(BaseModel):
    patient_identifier: Optional[str] = Field(default=None, description="Patient identifier, age, or demographics")
    subjective: Optional[str] = Field(default="Not documented", description="History of present illness, symptoms, pain scale, and duration")
    objective: Optional[str] = Field(default="Not documented / Non-contributory", description="Physical examination observations, vitals, auscultation, tenderness")
    assessment: Optional[str] = Field(default="Symptomatic review / Undifferentiated", description="Clinical impression and primary or differential diagnoses")
    plan_medications: List[Medication] = Field(default_factory=list, description="Prescribed medications")
    plan_instructions: List[str] = Field(
        default_factory=list,
        description="Non-pharmacological clinical instructions, lifestyle changes, referrals, or lab tests"
    ...
    safety_alerts: List[SafetyAlert] = Field(
        default_factory=list,
        description="Prescription safety flags: Drug-Allergy cross-reactivity, contraindications, or extreme dosages"
    )
...
async def process_voice_debrief(file: UploadFile = File(...)):
    ...
    try:
        ...
        segments, _ = whisper_model.transcribe(
            tmp_path,
            beam_size=5,
            initial_prompt="Clinical consultation, medical debrief, medications, anatomy, diagnosis, paracetamol, amoxicillin, dolo 650"
        )
        transcript_text = " ".join([seg.text for seg in segments]).strip()
        print(f"\n[Transcribed Audio]: {transcript_text}\n")
        ...
        # Step B: Clinical SOAP Extraction & Safety Auditing
        system_prompt = (
            "You are an expert clinical medical scribe and pharmacology safety auditor. "
            "Process the doctor's spoken transcript into a SOAP note and evaluate prescription safety.\n\n"
            "Rules:\n"
            "1. If a section (Objective exam or Assessment) was omitted, provide a concise summary or 'Not documented'. Never return null.\n"
            "2. Safety Checks (populate 'safety_alerts'):\n"
            "   - Allergy cross-reactivity (e.g. Amoxicillin/Penicillins prescribed with documented Penicillin allergy).\n"
            "   - Contraindications (e.g. NSAIDs in active GI bleeding or severe GERD; non-selective beta-blockers in severe asthma).\n"
            "   - Severe high-dosing frequencies.\n"
            "   - If no contraindications exist, return an empty array for 'safety_alerts'.\n"
            "3. Output strictly valid JSON matching this exact structure without markdown backticks:\n"
            "{\n"
            '  "patient_identifier": "string or null",\n'
            '  "subjective": "string",\n'
            '  "objective": "string",\n'
            ...
            '  "plan_instructions": ["string"],\n'
            '  "safety_alerts": [\n'
            '    {"severity": "HIGH or MODERATE", "medication": "string", "warning": "string"}\n'
            '  ]\n'
            "}"
        )
        user_prompt = f"Doctor's Voice Debrief Transcript:\n\"\"\"\n{transcript_text}\n\"\"\""

        print(f"[NVIDIA NIM] Auditing debrief & extracting SOAP...")
        completion = nvidia_client.chat.completions.create(
            model=NVIDIA_MODEL,
            messages=[
                {"role": "system", "content": system_prompt},
        ...
        print("[NVIDIA NIM] Extraction & safety evaluation complete.")
